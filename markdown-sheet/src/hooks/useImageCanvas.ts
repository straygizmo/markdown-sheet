...
export function useImageCanvas() {
  ...
  const getCanvasDataUrl = useCallback((region?: { left: number; top: number; width: number; height: number }) => {
    ...
    if (region) {
      // Crop directly from the background image to avoid coordinate mismatch
  ...
  useEffect(() => {
    ...
    // Clean up previous OCR selection rect
    if (ocrRect.current) {
      c.remove(ocrRect.current);
      ocrRect.current = null;
      c.renderAll();
    ...
    // Pen path created → push undo
    const handlePathCreated = () => { pushUndo(); };
  ...
  useEffect(() => {
    ...
    return () => { c.off("mouse:wheel", handleWheel); };
  }, []);
